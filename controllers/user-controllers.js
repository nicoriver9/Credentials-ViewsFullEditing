      ...
      .then(async (rows) => {
        ...
        if (role.role == "ADMIN_ROLE"){
                ...
                .then( async (users) => {
                    ...
                    mainUsers.addingTag = `<a href="/addRelative/:${mainUsers.id}">Create a Relative</a>`                    
    ...
    await db.query(
            ...
            .then(async (rows) => {
              ...
              if(rows[0].length >0){
                ...
                return res.status(200).render("credentialsEdit", { id });                
              }
...
const renderAddRelative = async (req, res) =>{
...
const renderAddMainUser = async (req, res) =>{

  //const { id } = req.params;      
  res.status(200).render('mainUsersAdd');
}

const addMainUser = async (req, res) =>{
  ...
  console.log(newMainUser);

  await db.query("INSERT INTO users set ?", [newMainUser]);
  req.flash("success", "MainUser Saved Successfully");
  res.redirect("/credentials");
}
